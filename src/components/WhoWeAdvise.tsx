          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {whoWeAdvise.groups.map((group, i) => (
              <RevealOnScroll as="li" key={group} delay={(i % 6) * 0.04}>
                <span className="block border-b border-e border-summit-graphite px-1 py-6 font-serif text-lg text-summit-ivory/90 transition-colors hover:text-summit-gold sm:text-xl">
                  {group}
                </span>
              </RevealOnScroll>
            ))}
